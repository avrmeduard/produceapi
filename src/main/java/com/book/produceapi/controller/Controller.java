...
public interface Controller {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = HttpServletResponse.SC_OK, message = "Result matching criteria", response = GetBookResponse.class),
            @ApiResponse(code = HttpServletResponse.SC_NOT_FOUND, message = "No entry found", response = GetBookResponse.class),
            @ApiResponse(code = HttpServletResponse.SC_BAD_REQUEST, message = "Invalid request", response = GetBookResponse.class),
    ...
    @ApiResponses(value = {
            @ApiResponse(code = HttpServletResponse.SC_CREATED, message = "Book added", response = GetBookResponse.class),
            @ApiResponse(code = HttpServletResponse.SC_CONFLICT, message = "Book already exist", response = GetBookResponse.class),
            @ApiResponse(code = HttpServletResponse.SC_BAD_REQUEST, message = "Invalid request", response = ErrorResponse.class),
            @ApiResponse(code = HttpServletResponse.SC_METHOD_NOT_ALLOWED, message = "Method not allowed", response = ErrorResponse.class)
    })
    @PostMapping("/addBook")
